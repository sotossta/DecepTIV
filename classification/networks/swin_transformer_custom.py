...
class Swin_Transformer_Custom(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, config, duration=8, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, thumbnail_rows=1, bottleneck=False):
        super().__init__()
        print(config)
        self.duration = config['clip_size']
